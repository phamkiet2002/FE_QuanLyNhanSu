...
import "./ListLeaveRegistrationNghiNgayStyle.scss";

const ListLeaveRegistrationNghiNgay = (props) => {
  ...
  const countPendingApproval = () => {
    return leaveRegistrations.filter(
      (item) => item.pendingApproval === "Chuaduyet"
    ).length;
  };
  ...
  const handleApprove = async (item) => {
    try {
      ...
      if (res.isSuccess) {
        ...
        toast.error(res.message || "Có lỗi xảy ra khi duyệt đơn");
      }
    } catch (error) {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      if (res.isSuccess) {
        ...
        toast.error(res.message || "Có lỗi xảy ra khi từ chối đơn");
      }
    } catch (error) {
  ...
  return (
    <>
      <div className="LeaveRegistration container">
        số lượng đơn chờ duyệt: {countPendingApproval()}
        <Table className="LeaveRegistration_table" striped bordered hover>
          ...
          <tbody>
            {leaveRegistrations.map((item, index) => (
            ...
            {leaveRegistrations && leaveRegistrations.length === 0 && (
              <tr>
...
export default ListLeaveRegistrationNghiNgay;
